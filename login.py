...
def search_ohthree(ohthree, regions, UUID):
    for i, region in enumerate(regions):
        url =  ohthree + '/%s/instance/%s/json' % (region.strip().lower(),
                                                   UUID)
...
if __name__ == '__main__':
    ...
    json = search_ohthree(values['ohthree'].strip('\''),
                          values['regions'].split(','),
                          args['<UUID>'])
    node = parse_node(json)

    login(values['bastion'].strip('\''), values['username'],
          values['password'], node)
